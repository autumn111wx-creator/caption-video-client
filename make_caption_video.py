...
def generate(
    ...
    if not (-50 <= rate <= 100):
        raise RuntimeError("语速必须在 0.5 到 2.0 倍之间。")
    if not (16 <= limit <= 80):
    ...
    progress("正在给背景视频添加字幕…" if background_video else "正在生成纯黑底字幕视频…")
    if background_video:
        video_input = ["-stream_loop", "-1", "-i", str(background_video)]
        video_filter = (
        ...
        video_input = ["-f", "lavfi", "-i", f"color=c=black:s=1920x1080:r=30:d={video_duration:.3f}"]
    ...
    execute(
        [
            ...
            *video_input,
            ...
            "-vf", video_filter, "-af", "apad=pad_dur=0.7",
